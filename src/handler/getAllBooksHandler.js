...
const booksSimple = (name, reading, finished) => {
	...
	if (name) {
		if (booksModule.length !== 0) {
			...
			data = SelectedData(booksModule, { name: pattern }, ["id", "name", "publisher"]);
		} else {
			data = [null];
		}
	}
	...
	if (reading) {
		if (booksModule.length !== 0) {
			if (parseInt(reading) === 1) {
				data = SelectedData(booksModule, { reading: true }, ["id", "name", "publisher"]);
			} else {
				data = SelectedData(booksModule, { reading: false }, ["id", "name", "publisher"]);
			}
		} else {
			data = [null];
		}
	...
	if (finished) {
		if (booksModule.length !== 0) {
			if (parseInt(finished) === 1) {
				data = SelectedData(booksModule, { finished: true }, ["id", "name", "publisher"]);
			} else {
				data = SelectedData(booksModule, { finished: false }, ["id", "name", "publisher"]);
			}
		} else {
			data = [null];
		}
